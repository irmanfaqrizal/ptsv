...
/* 
 * simpler [system] instance implementation 
 *
 */

if_simpler_instance::if_simpler_instance(if_pid_type pid, IfQueue* queue) 
  : IfInstance(pid, queue) {
} 

if_simpler_instance::if_simpler_instance(const if_simpler_instance& instance) 
  : IfInstance(instance) {
}
...
if_A_instance::if_A_instance() 
  : if_simpler_instance(if_pid_mk(if_A_process,0), IfQueue::NIL) {
  m_ctl.top=-1;
  m_ctl.top=1;
...
void if_A_instance::_wait_2_dispatch(IfMessage* message) {
  if (message != NULL)
    switch(message->getSid()) {
    }
  else {
    _wait_2_1_fire(message);
      ITERATOR->restore();
...
void if_A_instance::_wait_2_1_fire(IfMessage* X) {
  IfTime::Constraint constraints[] = 
    {{m_var.x,0,2*((int) 1)+1},{0,m_var.x,-2*(1)+1},{0,0,0}};
...
inline void if_A_instance::_wait_2_1a_fire() {

  ITERATOR->set(&m_var.x,0,m_pid);
}
inline void if_A_instance::_wait_2_1b_fire() {
...
void if_A_instance::_first_3_dispatch(IfMessage* message) {
  if (message != NULL)
    switch(message->getSid()) {
    }
  else {
    _first_3_1_fire(message);
      ITERATOR->restore();
...
void if_A_instance::_first_3_1_fire(IfMessage* X) {
  IfTime::Constraint constraints[] = 
...
inline void if_A_instance::_first_3_1a_fire() {

  ITERATOR->trace(IfEvent::INFORMAL, m_pid, "A", m_pid, (IfObject*)"A");
}
inline void if_A_instance::_first_3_1b_fire() {
...
if_B_instance::if_B_instance() 
  : if_simpler_instance(if_pid_mk(if_B_process,0), IfQueue::NIL) {
  m_ctl.top=-1;
  m_ctl.top=1;
...
void if_B_instance::_start_1_1_fire(IfMessage* X) {
  ...
  if (opt_gen_lineno) ITERATOR->trace(IfEvent::DEBUG, m_pid, "23"); 
...
void if_B_instance::_wait_2_dispatch(IfMessage* message) {
  if (message != NULL)
    switch(message->getSid()) {
    }
  else {
...
void if_B_instance::_wait_2_1_fire(IfMessage* X) {
  IfTime::Constraint constraints[] = 
    {{m_var.x,0,2*((int) 3)+1},{0,m_var.x,-2*(3)+1},{0,0,0}};
  ...
  if (opt_gen_lineno) ITERATOR->trace(IfEvent::DEBUG, m_pid, "27"); 
...
inline void if_B_instance::_wait_2_1a_fire() {
...
void if_B_instance::_first_3_dispatch(IfMessage* message) {
  if (message != NULL)
    switch(message->getSid()) {
    }
  else {
...
void if_B_instance::_first_3_1_fire(IfMessage* X) {
  IfTime::Constraint constraints[] = 
    {{0,0,0}};
...
inline void if_B_instance::_first_3_1a_fire() {

  ITERATOR->trace(IfEvent::INFORMAL, m_pid, "B", m_pid, (IfObject*)"B");
}
inline void if_B_instance::_first_3_1b_fire() {
